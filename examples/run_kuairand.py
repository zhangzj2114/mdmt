...
def get_kuai_rand(task, domain, data_path):
    data = pd.read_csv(data_path+"/kuai_rand.csv")

    x_used_cols = ['is_click', 'long_view', 'tab', 'user_active_degree', 'is_live_streamer', 'is_video_author', 'follow_user_num_range', 'fans_user_num_range', 'friend_user_num_range', 'register_days_range', 'onehot_feat0', 'onehot_feat1', 'onehot_feat2', 'onehot_feat3', 'onehot_feat4', 'onehot_feat5', 'onehot_feat6', 'onehot_feat7', 'onehot_feat8', 'onehot_feat9', 'onehot_feat10', 'onehot_feat11', 'onehot_feat12', 'onehot_feat13', 'onehot_feat14', 'onehot_feat15', 'onehot_feat16', 'onehot_feat17', 'video_type', 'upload_dt', 'upload_type', 'server_width', 'server_height', 'music_type', 'tag', 'counts']

    tab = [0, 1, 4]
    domain_indicator = {'0': 0, '1': 1, '4': 2}
    domain_num = 3
    
    # tab = [0, 1, 4, 2]
    # domain_indicator = {'0': 0, '1': 1, '4': 2, '2': 3}
    # domain_num = 4
    # tab = [0, 1, 4, 2, 6]
    # domain_indicator = {'0': 0, '1': 1, '4': 2, '2': 3, '6': 4}
    # domain_num = 5

    data = data[x_used_cols]
    
    mask_ = data['tab'].apply(lambda x: x in tab)
    data = data[mask_]
    
    data["domain_indicator"] = data["tab"].apply(lambda x: domain_indicator[str(x)])
    del data['tab']

    tab_c = Counter(data['domain_indicator'])
    print(tab_c)
    # choose domain: 0, 1, 2, all
    if domain != 'all':
    ...
    useless_features = []
    dense_features = []
    sparse_features = ['user_active_degree', 'is_live_streamer', 'is_video_author', 'follow_user_num_range', 'fans_user_num_range', 'friend_user_num_range', 'register_days_range', 'onehot_feat0', 'onehot_feat1', 'onehot_feat2', 'onehot_feat3', 'onehot_feat4', 'onehot_feat5', 'onehot_feat6', 'onehot_feat7', 'onehot_feat8', 'onehot_feat9', 'onehot_feat10', 'onehot_feat11', 'onehot_feat12', 'onehot_feat13', 'onehot_feat14', 'onehot_feat15', 'onehot_feat16', 'onehot_feat17', 'video_type', 'upload_dt', 'upload_type', 'server_width', 'server_height', 'music_type', 'tag', 'counts'] + ['domain_indicator']
    ...
    # assert set(data['is_click']) == {0, 1}
    # assert set(data['long_view']) == {0, 1}
    # assert set(data['is_like']) == {0, 1}
    # assert set(data['is_follow']) == [0, 1]
    # assert set(data['is_comment']) == [0, 1]
    # assert set(data['is_forward']) == [0, 1]
    # assert set(data['is_hate']) == [0, 1]
    ...
    sparse_feas = [SparseFeature(sparse_features.index(feature_name), vocab_size=data[feature_name].nunique(), embed_dim=16) for feature_name in sparse_features]

    # task: click, like, all
    # data.rename(columns={'is_click': 'click', 'is_like': 'like'}, inplace=True)
    # if task == 'all':
    #     y = data[['click', 'like']]
    # else:
    #     y = data[task]
    ...
    # task: click, long_view, all
    data.rename(columns={'is_click': 'click', 'long_view': 'long'}, inplace=True)
    # data.rename(columns={'is_click': 'click', 'is_like': 'like'}, inplace=True)
    if task == 'all':
        y = data[['click', 'long']]
    else:
...
def main(args):
    ...
    dense_feas, sparse_feas, x_train, y_train, x_val, y_val, x_test, y_test, domain_num = get_kuai_rand(args.task, args.domain, args.dataset_path)
    if args.domain != 'all':
        domain_num = 1
    ...
    col_name_ = [f'auc_{i}' for i in col_name_] + [f'log_{i}' for i in col_name_] + ['auc', 'log', 'seed', 'n_w', 'sd_w', 'st_w', 't_w', 'sigma_w']
    # for t in range(task_num):
    #     for d in range(domain_num):
    ...
    # save csv file
    import csv
    file_name_ = f'output/{args.model_name}_kuairand{domain_num}_domain_{args.domain}_task_{args.task}_exp_{args.expert_num}.csv'
    print_round_ = 4
    ...
    with open(file_name_, mode_) as f:
        ...
        writer.writerow([round(auc_, print_round_) for auc_ in auc_row_] + 
                        ...
                        [round(total_auc, print_round_), round(total_logloss, print_round_), 
                         args.seed, args.n_w, args.sd_w, args.st_w, args.t_w, args.sigma_w]
                       )
        # writer.writerow([round(auc_, print_round_) for auc_ in auc_list] + 
        #                 [round(log_, print_round_) for log_ in logloss_list] + 
        #                 [round(total_auc, print_round_), round(total_logloss, print_round_)]
        #                )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', default="../../data/kuai-rand")
    ...
    parser.add_argument('--task', type=str, default='all', choices=['click', 'long', 'all'])
    parser.add_argument('--domain', type=str, default='all', choices=['0', '1', '2', '3', 'all'])
