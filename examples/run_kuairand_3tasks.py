...
def get_kuai_rand(task, domain, data_path):
    ...
    # task: click, like, all
    data.rename(columns={'is_click': 'click', 'long_view': 'long', 'is_like': 'like'}, inplace=True)
    if task == 'all':
        y = data[['click', 'long', 'like']]
    else:
        y = data[task]
    del data['click']
    del data['long']
...
def main(args):
    ...
    if args.domain != 'all':
        domain_num = 1
    task_num = 3 if args.task == 'all' else 1
    ...
    # save csv file
    import csv
    file_name_ = f'output/{args.model_name}_kuairand3t{domain_num}d_domain_{args.domain}_task_{args.task}_exp_{args.expert_num}.csv'
    print_round_ = 4
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default='all', choices=['click', 'long', 'like', 'all'])
